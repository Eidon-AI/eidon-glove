...
static void ble_gap_event_handler(esp_gap_ble_cb_event_t event, esp_ble_gap_cb_param_t *param)
{
    ...
    case ESP_GAP_BLE_ADV_DATA_SET_COMPLETE_EVT:
        ESP_LOGI(TAG, "BLE GAP ADV_DATA_SET_COMPLETE: status = %d", param->adv_data_cmpl.status);
        if (param->adv_data_cmpl.status != ESP_BT_STATUS_SUCCESS) {
            ESP_LOGE(TAG, "Advertising data set failed!");
        }
        break;

    case ESP_GAP_BLE_ADV_START_COMPLETE_EVT:
        ESP_LOGI(TAG, "BLE GAP ADV_START_COMPLETE: status = %d", param->adv_start_cmpl.status);
        if (param->adv_start_cmpl.status != ESP_BT_STATUS_SUCCESS) {
            ESP_LOGE(TAG, "Advertising start failed!");
        } else {
            ESP_LOGI(TAG, "Device is now discoverable and advertising!");
        }
        break;
...
esp_err_t esp_hid_ble_gap_adv_start(void)
{
    static esp_ble_adv_params_t hidd_adv_params = {
        ...
        .adv_int_max        = 0x40,  // 40ms - increased for better discovery
    ...
    // Get and log the device address
    uint8_t addr[6] = {0};
    uint8_t addr_type;
    esp_err_t addr_ret = esp_ble_gap_get_local_used_addr(addr, &addr_type);
    if (addr_ret == ESP_OK) {
